...
export default function MetricsHeader({ onFiltersChange }: MetricsHeaderProps) {
  ...
  const handleDateChange = (value: string) => {
    ...
    toast.info('Filtre aplicat', {
      description: `Mostrant dades per: ${value === 'last7' ? 'Últims 7 dies' : value === 'last30' ? 'Últims 30 dies' : value === 'last90' ? 'Últims 90 dies' : 'Any 2026'}`,
    });
  };
  ...
  const handleTechnicianChange = (value: string) => {
    ...
    if (value !== 'all') {
      toast.info('Filtre tècnic aplicat', {
        description: `Mostrant dades de: ${value === 'marc' ? 'Marc Puigdomènech' : value === 'laia' ? 'Laia Ferré' : value === 'jordi' ? 'Jordi Casals' : value === 'nuria' ? 'Núria Valls' : 'Pau Ribas'}`,
      });
    }
  };
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      ...
      toast.success('Mètriques actualitzades', {
        description: `Dades actualitzades a ${new Date().toLocaleTimeString('ca-ES')}`,
      });
    }, 1000);
  };
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Observabilitat operativa · Etecnic Camp de Tarragona
          </p>
        </div>
      </div>
