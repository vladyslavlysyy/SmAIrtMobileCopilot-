...
import { AlertTriangle, X, Car, Package, Clock, MessageSquare, Send, ChevronDown } from 'lucide-react';
import { toast } from 'sonner';
import Icon from '@/components/ui/AppIcon';


const contingencyTypes = [
  { id: 'traffic', label: 'Trànsit / Accident', icon: Car, color: 'text-red-600', bg: 'bg-red-50', border: 'border-red-200' },
  { id: 'material', label: 'Falta de material', icon: Package, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-200' },
  { id: 'delay', label: 'Retard en visita', icon: Clock, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200' },
  { id: 'other', label: 'Altre imprevisto', icon: MessageSquare, color: 'text-slate-600', bg: 'bg-slate-50', border: 'border-slate-200' },
];
          ...
          <div className="bg-card rounded-2xl border border-border w-full max-w-md shadow-2xl animate-slide-up">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mb-2">Quants minuts de retard preveus?</p>
              ...
              <div className="flex items-start gap-2.5 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                ...
                <p className="text-xs text-blue-700">
                  Operacions rebrà l&apos;avís immediatament i proposarà alternatives de replantejament per a les visites afectades.
                </p>
              </div>
            </div>
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold text-white rounded-xl transition-all duration-150 active:scale-95
                  ${isSending || !selectedType
                    ? 'bg-red-400 cursor-not-allowed' :'bg-red-600 hover:bg-red-700'
                  }`}
              >
                {isSending ? (
                  <>
