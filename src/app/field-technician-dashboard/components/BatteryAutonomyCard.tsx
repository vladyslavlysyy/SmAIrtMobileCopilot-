...
const routeLegs = [
  { id: 'leg-1', from: 'Inici (Tarragona)', to: 'CST Oncè de Setembre', km: 0, batteryStart: 92, batteryEnd: 89 },
  { id: 'leg-2', from: 'CST Oncè de Setembre', to: 'CST Av. Roma', km: 3, batteryStart: 89, batteryEnd: 84 },
  { id: 'leg-3', from: 'CST Av. Roma', to: 'Hotel Salou', km: 22, batteryStart: 84, batteryEnd: 72 },
  { id: 'leg-4', from: 'Hotel Salou', to: 'Recàrrega Salou', km: 5, batteryStart: 72, batteryEnd: 55 },
  { id: 'leg-5', from: 'Recàrrega Salou', to: 'Ajunt. Tarragona', km: 18, batteryStart: 85, batteryEnd: 72 },
  { id: 'leg-6', from: 'Ajunt. Tarragona', to: 'Polígon Vila-seca', km: 8, batteryStart: 72, batteryEnd: 58 },
  { id: 'leg-7', from: 'Polígon Vila-seca', to: 'Residencial Ponent', km: 12, batteryStart: 58, batteryEnd: 45 },
];
  ...
  return (
    <div className="bg-card rounded-xl border border-border overflow-hidden">
      <div className="px-4 py-3 border-b border-border">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className={`text-xl font-bold font-mono tabular-nums ${getBatteryTextColor(currentBattery)}`}>
              {currentBattery}%
            </p>
        ...
        <div className="mt-3">
          ...
          {minBattery < 30 && (
            <div className="flex items-center gap-1.5 mt-2 text-xs text-amber-700">
              ...
              <span>Bateria mínima prevista: <span className="font-bold font-mono">{minBattery}%</span> — Recàrrega planificada</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-4 space-y-2">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Consum per tram</p>
        {routeLegs.map((leg) => {
            ...
            <div key={leg.id} className="flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground ml-auto flex-shrink-0 font-mono">{leg.km}km</span>
                </div>
              ...
              <div className="flex-shrink-0 text-right w-16">
                <span className={`text-xs font-mono font-semibold tabular-nums ${isRecharge ? 'text-cyan-600' : getBatteryTextColor(leg.batteryEnd)}`}>
                  {isRecharge ? `+${delta}%` : `${delta}%`}
                </span>
                <p className="text-xs font-mono tabular-nums text-muted-foreground">{leg.batteryEnd}%</p>
              </div>
            </div>
          );
        })}
