...
import { Zap, BarChart3, Building2, HardHat, ArrowRight, Shield, Clock, MapPin } from 'lucide-react';
  ...
  const roles = [
    {
      ...
      color: 'primary',
      features: ['Cua de tasques prioritzada per IA', 'Mapa de punts de recàrrega', 'Gestió d\'imprevistos', 'Planificació de jornades'],
      badge: 'Desktop-first',
    },
    {
      id: 'technician',
      ...
      color: 'secondary',
      features: ['Ruta del dia amb autonomia VE', 'Calendari setmanal', 'Formularis d\'intervenció', 'Reportar imprevistos'],
      badge: 'Mobile-first',
    },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 flex flex-col">
      ...
      <header className="px-6 py-5 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          <AppLogo size={36} />
          <div>
            <span className="font-semibold text-white text-lg tracking-tight">SmAIrt Mobility Copilot</span>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="text-center mb-12 animate-fade-in">
          ...
          <p className="text-blue-200 text-lg max-w-xl mx-auto">
            El copilot d&apos;IA s&apos;adapta a les teves necessitats. Tria la vista corresponent al teu perfil.
          </p>
        </div>
            ...
            return (
              <button
                ...
                className={`relative text-left p-7 rounded-2xl border transition-all duration-300 group
                  ${isHovered
                    ? role?.color === 'primary' ?'bg-blue-600/30 border-blue-400/60 shadow-2xl shadow-blue-500/20 scale-[1.02]' :'bg-cyan-600/30 border-cyan-400/60 shadow-2xl shadow-cyan-500/20 scale-[1.02]' :'bg-white/5 border-white/15 hover:border-white/30'
                  }`}
              >
                <div className="flex items-start justify-between mb-5">
                  <div className={`p-3 rounded-xl ${role?.color === 'primary' ? 'bg-blue-500/30' : 'bg-cyan-500/30'}`}>
                    <Icon size={28} className={role?.color === 'primary' ? 'text-blue-300' : 'text-cyan-300'} />
                  </div>
                ...
                <p className={`text-sm font-medium mb-3 ${role?.color === 'primary' ? 'text-blue-400' : 'text-cyan-400'}`}>
                  {role?.subtitle}
                </p>
                <p className="text-slate-300 text-sm leading-relaxed mb-5">{role?.description}</p>
                <ul className="space-y-2 mb-6">
                  {role?.features?.map((feature) => (
                    <li key={`feat-${role?.id}-${feature}`} className="flex items-center gap-2 text-sm text-slate-300">
                      <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${role?.color === 'primary' ? 'bg-blue-400' : 'bg-cyan-400'}`}></span>
                      {feature}
                    </li>
                  ))}
                </ul>
                <div className={`flex items-center gap-2 font-semibold text-sm transition-all duration-150
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-4xl">
          {stats?.map((stat) => {
            ...
            return (
              <div key={`stat-${stat?.label}`} className="flex items-center gap-3 p-4 rounded-xl bg-white/5 border border-white/10">
                <Icon size={20} className="text-blue-400 flex-shrink-0" />
                <div>
                  <p className="text-white font-bold text-xl tabular-nums font-mono">{stat?.value}</p>
      ...
      <footer className="px-6 py-4 border-t border-white/10 text-center">
        <p className="text-slate-500 text-xs">
          SmAIrt Mobility Copilot · Etecnic Mobilitat Elèctrica, SL · Prototip demostratiu — no reflecteix sistemes reals
        </p>
      </footer>
    </div>
  );
